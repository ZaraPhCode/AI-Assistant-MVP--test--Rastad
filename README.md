...
***نکته فنی***: در `classifier.py`، تابع `_classify_with_llm()` از `llm_service.detect_intent()` استفاده می‌کند که مستقیماً `Claude API` را با یک system prompt طبقه‌بندی intent فراخوانی می‌کند. Claude فقط نام intent را برمی‌گرداند (بدون توضیح اضافه). در صورت خطای API، "unknown" برگردانده می‌شود و کاربر پاسخ عمومی دریافت می‌کند.
